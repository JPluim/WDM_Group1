...
MAX_REVISIONS = 1500000
FILE_PATH = './data/web_data_management_public_revision_user_talk.csv'
REVISION_TYPE = 'user_talk'
...
def process_revision(revision)
  ...
  @bulk_body << { create: { _index: 'user_talk_revisions', _type: 'revision', _id: revision["id"], data: { doc: revision } } }
end
...
CSV.foreach(FILE_PATH, liberal_parsing: true, headers: true, quote_char: '|') do |row|
  ...
  if @bulk_body.size >= MAX_BULK_BODY_SIZE
      bulk_update
      # break if @revisions_processed >= MAX_REVISIONS
  end
end

bulk_update
puts 'Total user talk revisions processed: ' + @revisions_processed.to_s
